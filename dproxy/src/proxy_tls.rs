...
use tokio::{io::{self,AsyncReadExt, AsyncWriteExt}, net::{TcpStream, TcpListener}};
...
async fn get_sni(sock: &mut TcpStream) -> Result<(Vec<u8> , Vec<u8>), anyhow::Error> {
    ...
    let (_, packet) = loop {
        ...
        if n == 0 {
            bail!("TLS parsing hit EOF bufore completion");
        }
    };
...
async fn real_proxy_tls(mut sock: TcpStream) -> AResult<()> {
    ...
    sock.read_exact(&mut authbuf).await.context("sock.read_exact")?;
    ...
    server_name += ":443";
    if !crate::auth::vertify_auth(auth, &server_name) {
        bail!("auth failed");
    }
    tracing::info!("auth passed");

    let mut other_side = TcpStream::connect(&server_name)
        .await
        .with_context(|| format!("TcpStream::connect({server_name})"))?;
    other_side.write_all(&head)
        .await
        .context("other_side.write_all(&head)")?;
    tracing::debug!("calling copy_bidireactional");
